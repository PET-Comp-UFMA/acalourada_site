<?php
    require_once('html_header.php');
    require_once('header.php');
?>

    <!--==========================
            Programação Geral
        ============================-->
        

        <section id="schedule" class="section-with-bg">
            <div class="container wow fadeInUp">
                <div class="section-header">
                    <h2>PROGRAMAÇÃO DO EVENTO</h2>
                </div>

                <!--  <h3 class="sub-heading">Organização - PETComp</h3>-->
                <div class="tab-content row justify-content-center">
                    
                    <!-- Schdule Day 1 -->
                    <div role="tabpanel" class="col-lg-9 tab-pane fade show active" id="day-1">
                        <h2 class="schedule-day-title">Dia 08/06</h2>
                        <div class="row schedule-item">
                            <div class="col-md-2">
                                <time>09:00-09:30</time>
                            </div>
                            <div class="col-md-10">
                                <div class="speaker">
                                    <img src="img/speakers/paulo_victor.png" alt="Paulo Victor">
                                </div>
                                <div class="speaker">
                                    <img src="img/speakers/tutoriais/Arthur Passos.jpg" alt="Arthur Passos<">
                                </div>
                                <h4>Dinâmica de apresentação</h4>
                                <p>Apresentador(a): Paulo Victor e Arthur Passos</p>
                            </div>
                        </div>
                        <div class="row schedule-item">
                            <div class="col-md-2">
                                <time>09:30-11:00</time>
                            </div>
                            <div class="col-md-10">
                                <div class="speaker">
                                    <img src="img/speakers/AnselmoPerfil.jpg" alt="Luis Rivero">
                                </div>
                                <div class="speaker">
                                    <img src="img/speakers/fundo.png" alt="">
                                </div>
                                <h4>Palestra / apresentação NCA</h4>
                                <p>Apresentador(a): Prof. Anselmo de Paiva</p>
                            </div>
                        </div>
                        <div class="row schedule-item">
                            <div class="col-md-2">
                                <time>11:00-12:00</time>
                            </div>
                            <div class="col-md-10">
                                <div class="speaker">
                                    <img src="img/speakers/DEINF.png"PETComp">
                                </div>
                                <div class="speaker">
                                    <img src="img/speakers/fundo.png" alt="">
                                </div>
                                <h4>Apresentação da coordenação / curso</h4>
                                <p>Apresentador(a): Coordenadora Simara</p>
                            </div>
                        </div>

                        <!-- Schdule Day 2 -->
                        <h2 class="schedule-day-title">Dia 09/06</h2>
                        <div class="row schedule-item">
                            <div class="col-md-2"><br>
                                <time> 09:00-09:30</time>
                            </div>
                            <div class="col-md-10">
                                <div class="speaker">
                                    <img src="img/speakers/tutoriais/Arthur Passos.jpg"PETComp">
                                </div>
                                <div class="speaker">
                                    <img src="img/speakers/paulo_victor.png" alt="Paulo Victor">
                                </div>
                                
                                <h4>Apresentação PETComp</h4>
                                <p>Apresentador(a): Arthur Passos e Paulo Victor</p>
                            </div>                          
                        </div>
                        <div class="row schedule-item">
                            <div class="col-md-2"><br>
                                <time> 09:30-10:00</time>
                            </div>
                            <div class="col-md-10">
                                <div class="speaker">
                                    <img src="img/speakers/lucho.jpg" alt="Luis Rivero">
                                </div>
                                <div class="speaker">
                                    <img src="img/speakers/fundo.png" alt="">
                                </div>
                                <h4>Apresentação da SBC</h4>
                                <p>Apresentador(a): Prof. Luis Rivero</p>
                            </div>                          
                        </div>
                        <div class="row schedule-item">
                            <div class="col-md-2"><br><br>
                                <time>10:00-11:30</time>
                            </div>
                            <div class="col-md-10">
                                <div class="speaker">
                                    <img src="img/speakers/jessica.png".jpg" alt="Nathasha Araújo">
                                </div>
                                <div class="speaker">
                                    <img src="img/speakers/arthur.png" alt="">
                                </div><br>

                                <h4>Mesa redonda com egressos</h4>
                                <p>Apresentador(a): Egressos de computação</p>

                                <div class="speaker">
                                    <img src="img/speakers/gabriel.jpeg" alt="">
                                </div>
                                <div class="speaker">
                                    <img src="img/speakers/durand.jpeg" alt="">
                                </div>
                            </div>
                        </div>

                        <!-- Schdule Day 3 -->
                        <h2 class="schedule-day-title">Dia 10/06</h2>
                        <div class="row schedule-item">
                            <div class="col-md-2">
                              <time>09:00-09:30</time>
                          </div>
                          <div class="col-md-10">
                              <div class="speaker">
                                <img src="img/labs/dexters.jpg" alt="Dexters">
                              </div>
                              <div class="speaker">
                                  <img src="img/speakers/fundo.png" alt="">
                              </div>
                              <h4>LabTour - Dexters</h4>
                              <p>Apresentador(a): Prof. Luis Rivero</p>
                          </div>
                          </div>
                        <div class="row schedule-item">
                          <div class="col-md-2">
                            <time>09:30-10:00</time>
                          </div>
                          <div class="col-md-10">
                            <div class="speaker">
                                <img src="img/labs/lacmor.png" alt="Lacmor" >
                            </div>
                            <div class="speaker">
                                <img src="img/speakers/fundo.png" alt="">
                            </div>
                            <h4>LabTour - LACMOR </h4>
                            <p>Apresentador(a): Prof. Alexandre César</p>
                          </div>
                        </div>
                        <div class="row schedule-item">
                            <div class="col-md-2">
                              <time>10:00-10:30</time>
                          </div>
                          <hr/>
                          <div class="col-md-10">
                              <div class="speaker">
                                <img src="img/labs/laws.png" alt="Laws">
                              </div>
                              <div class="speaker">
                                  <img src="img/speakers/fundo.png" alt="">
                              </div>
              
                              <h4>LabTour - LAWS</h4>
                              <p>Apresentador(a): Priscila Brito</p>                             
                        </div>
                        </div>

                        <div class="row schedule-item">
                            <div class="col-md-2">
                              <time>10:30-11:00</time>
                          </div>
                          <hr/>
                          <div class="col-md-10">
                            <div class="speaker">
                              <img src="img/labs/telemidia.png" alt="telemidia">
                            </div>
                            <div class="speaker">
                                <img src="img/speakers/fundo.png" alt="">
                            </div>
              
                              <h4>LabTour - Telemídia</h4>
                              <p>Apresentador(a): Priscila Brito</p>   
                        </div>
                        </div>
                          
                        <!-- -->

                          <div class="row schedule-item">
                            <div class="col-md-2">
                              <time>11:00-11:30</time>
                          </div>
                          <hr/>
                          <div class="col-md-10">
                            <div class="speaker">
                              <img src="img/labs/viplab.png" alt="">
                            </div>
                            <div class="speaker">
                                <img src="img/speakers/fundo.png" alt="">
                            </div>
              
                              <h4>VIPLab</h4>
                              <p>Apresentador(a): Prof° Geraldo Braz</p>
                              
                          </div>
                          </div>

                          <div class="row schedule-item">
                            <div class="col-md-2">
                              <time>11:30-12:00</time>
                          </div>
                          <div class="col-md-10">
                              <div class="speaker">
                                <img src="img/labs/lsdi.png" alt="LSDi">
                              </div>
                              <div class="speaker">
                                  <img src="img/speakers/fundo.png" alt="">
                              </div>
                              <h4>LabTour - LSDi</h4>
                              <p>Apresentador(a): Prof. Francisco Silva</p>
                          </div>
                          </div>


                        
                        <!-- Schdule Day 4 -->
                        <h2 class="schedule-day-title">Dia 11/06</h2>
                        <div class="row schedule-item">
                          <div class="col-md-2">
                            <time>09:00-09:30</time>
                          </div>
                          <div class="col-md-10">
                            <div class="speaker">
                                <img src="img/speakers/logo_connection.png" alt="Connection" >
                            </div>
                            <div class="speaker">
                                <img src="img/speakers/lorde.png" alt="Lorde" >
                            </div>
                            <h4>Apresentação da Connection/
                                Apresentação da Atlética</h4>
                            <p>Apresentador(a): Presidente Mylady e Lorde</p>
                          </div>
                        </div>
                            
                        <!-- -->
                        <div class="row schedule-item">
                          <div class="col-md-2">
                            <time>09:30-10:00</time>
                        </div>
                        <div class="col-md-10">
                            <div class="speaker">
                              <img src="img/speakers/nathasha.jpg" alt="pet">
                            </div>
                            <div class="speaker">
                                <img src="img/speakers/fundo.png" alt="">
                            </div>
                            <h4>Apresentação - Girls++</h4>
                            <p>Apresentador(a): Nathasha Pinto</p>
                        </div>
                        </div>
                            
                        <!-- -->
                        <div class="row schedule-item">
                          <div class="col-md-2">
                            <time>10:00-11:00</time>
                        </div>
                        <div class="col-md-10">
                            <div class="speaker">
                              <img src="img/speakers/andre.jpg" alt="">
                            </div>
                            <div class="speaker">
                                <img src="img/speakers/paulo_victor.png" alt="">
                            </div>
                            <h4>Dinâmica do Kahoot</h4>
                            <p>Apresentador(a): André Barreto e Paulo Victor</p>
                        </div>
                        </div>

                        <!-- -->
                        <div class="row schedule-item">
                            <div class="col-md-2">
                              <time>11:00-11:30</time>
                          </div>
                          <div class="col-md-10">
                              <div class="speaker">
                                <img src="img/speakers/andre.jpg" alt="">
                              </div>
                              <div class="speaker">
                                  <img src="img/speakers/tutoriais/Arthur Passos.jpg" alt="Arthur Passos<">
                              </div>
                              <h4>Encerramento e premiação</h4>
                              <p>Apresentador(a): André Barreto e Arthur Passos</p>
                          </div>
                          </div>



                         <!-- -->
                         <div class="row schedule-item">
                          <div class="col-md-2">
                            <time>20:00</time>
                        </div>
                        <div class="col-md-10">
                            <div class="speaker">
                              <img src="img/speakers/pet.jpg"alt ="PETComp">
                            </div>
                            <div class="speaker">
                                <img src="img/speakers/fundo.png" alt="">
                            </div>
                            <h4>Sessão de jogos</h4>
                            <p>Apresentador(a): Aberto</p>
                        </div>
                        </div>

                          </div>
                        </div>
                    </div>
                </div>

            </div>


        </section>

<?php 
    require_once('footer.php');
    require_once('html_footer.php');
?>